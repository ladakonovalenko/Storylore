...
export default function CharactersPage() {
  ...
  const allTags = useMemo(() => {
    const set = new Set()
    characters.forEach((c) => { if (Array.isArray(c.tags)) c.tags.forEach((t) => set.add(t)) })
    return [...set].sort()
  }, [characters])
  ...
  const filtered = useMemo(() => {
    ...
    if (activeProjectId) {
      list = list.filter((c) => !c.project_id || String(c.project_id) === String(activeProjectId))
    }
    if (search.trim()) {
    ...
    if (filterTag)    list = list.filter((c) => Array.isArray(c.tags) && c.tags.includes(filterTag))

    return list
  }, [characters, search, filterStatus, filterTag, activeProjectId])
